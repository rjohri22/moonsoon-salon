                    ...
                    <div class="row">
                        <div class="col">
                            <div class="float-left">
                                <h3 class="h-p-bold-gray">Service Order</h3>
                            </div>
                                            ...
                                            <tr id="pId{{ $data->id }}">
                                                ...
                                                <td>
                                                    <a href="javascript:void(0)" class="btn btn-sm btn-primary"
                                                        onclick="viewService({{ $data->id }})">View</a>
                                                        <a href="javascript:void(0)" class="btn btn-sm btn-primary"
                                                    onclick="downloadOrder({{ $data->id }})">Download</a>
                                                    <a href="{{ route('admin-service-orders.destroy', $data->id) }}"
        ...
        <div class="modal-dialog  modal-lg" role="document">
            <div class="modal-content"> 
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-4">
                            <label for="view_service_order_no">Order No : -</label>
                            <span id="view_service_order_no"></span>
                        </div>
                        <div class="col-md-4">
                            <label for="view_booking_order_date_time_display">Booking Date & Time :- </label>
                            <span id="view_booking_order_date_time_display"></span>
                        </div>
                        <div class="col-md-4">
...
@section('js')
    ...
    <script>
        ...
        function viewService(id) {
            ...
            $.ajax({
                url: "{{ url('admin/service-orders') }}" + "/" + id,
                method: "GET",
                success: function(response) {
                    console.log("response",response);
                    $('#view_service_order_no').html(response.data.service_order_no);
                    $('#view_booking_order_date_time_display').html(response.data.booking_order_date_time_display);
                    ...
                    let toAppend = '';
                    $( response.data.service_order_items).each(function( index ,value) {
                        // console.log( index + ": " + $( this ).text() );
                        toAppend +=`<tr><td>`+value.service_name+`</td><td>`+value.rate+`</td><td>`+value.service_date_display+`</td><td>`+value.service_time_display+`</td><td>`+value.total_display+`</td></tr>`;
                    });
        ...
        function downloadOrder(id) {
            $.ajax({
                url: "{{ url('admin/genrate-service-order-invoice') }}" + "?service_orders_id=" + id,
                method: "GET",
                success: function(response) {
                    console.log("response",response);
