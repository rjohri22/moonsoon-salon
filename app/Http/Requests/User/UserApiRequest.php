...
class UserApiRequest extends FormRequest
{
    ...
    {
        return [
            "name" => "max:100|required_if:user_type,user",
            'username' => "required_unless:user_type,user",
            "email" => "email | max:100 | unique:users",
            "phone_no" => "min:10 | max:15 | unique:users"
        ];
    }
    ...
    public function failedValidation(Validator $validator)
    {
        ...
        throw new HttpResponseException(response()->json(['status' => 200, 'message' => /* (string)json_encode */ implode(" ", $errors[$keys[0]]), 'extra' => 'validation errors'], 200));
    }
}
